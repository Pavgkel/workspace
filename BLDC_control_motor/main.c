...
int main(void)
{
	// clock setup
	SetSysClockTo72();
	RCC_APB1PeriphClockCmd(RCC_AHBPeriph_GPIOA | RCC_AHBPeriph_GPIOB | RCC_AHBPeriph_GPIOC ,ENABLE);
...
//	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinSource9);
	...
	// EXTI
	EXTI_InitStruct.EXTI_Line = EXTI_Line3 | EXTI_Line5 | EXTI_Line7;
...
void adc_init(void)
{
	...
	//ADC_InitStructure.ADC_ExternalTrigConvEvent = ADC_ExternalTrigConv_None;
	ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;
	ADC_InitStructure.ADC_NbrOfRegChannel = 1;

	ADC_RegularChannelConfig(ADC1, ADC_Channel_9, 1, ADC_SampleTime_19Cycles5); // define regular conversion config
...
//	ADC_InitStructure.ADC_Mode = ADC_Mode_Independent;
	...
	// start conversion
	ADC_Cmd(ADC1, ENABLE);					//enable ADC1
	ADC_StartConversion(ADC1); // start conversion (will be endless as we are in continuous mode)
}
...
void HallSensorsGetPosition(void)
{
	uint8_t hallpos = (uint8_t)((GPIO_ReadInputData(GPIOB) & (GPIO_Pin_3 | GPIO_Pin_5 | GPIO_Pin_7)) >> 7);
	switch (hallpos)
	{
	case 0b101:
		phase = 0;
